...
export const PlaceChatModal = React.memo<PlaceChatModalProps>(({
  visible,
  onClose,
  roomName,
  channelKey,
  ...
  const handleLongPressBubble = useCallback((msg: PlaceChatMessage) => {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Cancel', style: 'cancel', onPress: () => setReportingMsgId(null) },
      ],
    );
    ...
    <Modal visible={visible} animationType="slide" transparent>
      ...
      >
        ...
        <View style={styles.card}>
          ...
          <View style={styles.header}>
            <View>
              <Text style={styles.title}>Chat in {roomName}</Text>
              {chatMode === 'friends_only' && (
                <Caption style={styles.chatModeHint}>Friends only</Caption>
              )}
              {isSafeChatOnly && (
                <Caption style={styles.chatModeHint}>Quick phrases only</Caption>
              )}
            </View>
            <TouchableOpacity onPress={onClose} hitSlop={12}>
              <Icon name="close" size={22} color={colors.text.muted} />
          ...
          <Caption style={styles.hint}>
            {chatMode === 'friends_only'
              ? 'Only friends can see your messages'
              : 'Keep it kind and fun!'}
          </Caption>
        </View>
      </KeyboardAvoidingView>
    </Modal>
...
const styles = StyleSheet.create({
  ...
  chatModeHint: { fontSize: 11, color: colors.primary.wisteria, marginTop: 1 },
  messageList: { maxHeight: 260, zIndex: 1 },
  ...
  phraseChipText: {
    fontSize: 13, fontWeight: '600', color: colors.primary.wisteriaDark,
  },

  safeChatOnlyBar: {
    marginTop: spacing.xs, marginBottom: spacing.xs, zIndex: 1,
  },
  safeChatToggle: {
  ...
  safeChatToggleText: {
    fontSize: 14, fontWeight: '600', color: colors.primary.wisteriaDark,
  },
  ...
  hint: { textAlign: 'center', marginTop: spacing.xs, color: colors.text.muted, zIndex: 1 },
});
